...
public class lab2 {
    public static void main(String[] args) {
        ...
        try {
            ...
            //init your classroom with 0's healthy, X infected and present, -means absent, /means infected and dying (but printed as -), : means dead
    ...
    public static char getNextInfectedState(char curState, boolean inSameRow) {
        switch (curState) {
            case 'O':       // if healthy, infect, if also in same rown, dying
                return inSameRow ? '/' : 'X';
            case '/':       // if dying, die
                return ':';
    ...
    public static char[][] processInfections(char[][] seats) {
        ...
        // iterates through array looking for an X and infects adjacent students
        for (int i = 0; i < newSeats.length - 1; ++i) {
            for (int j = 0; j < newSeats[i].length - 1; ++j) {
                if (newSeats[i][j] == 'X') {
                    newSeats[i + 1][j] = 'X';
                    newSeats[i - 1][j] = 'X';
                    newSeats[i][j + 1] = 'X';
    ...
    /**
     * Initializes all the seats to initState: Os healthy, X means infected and present,
     * - means absent, / means infected and dying (but printed as -),
     * : means dead (but printed as -)
     *
     * @param initState
     * @return 4x10 two dimensional arrays of Os or healthy students
     */
    public static char[][] initSeats(char initState) {
        char[][] seats = new char[4][10];

        // initialize array with student Os
